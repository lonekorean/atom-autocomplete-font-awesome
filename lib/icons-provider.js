...
class IconsProvider {
	constructor() {
		this.selector = '.text.html .meta.tag .string.quoted, .source.js .string.quoted';
	...
	createSuggestion(prefixBase, icon) {
		...
		return {
			type: 'value',
			iconHTML: '<i class="aafa-fa fa ' + className + '"></i>',
			displayText: className + (extraTerm ? ' (' + extraTerm + ')' : ''),
			text: className,
			rightLabel: '\\' + icon.unicode,
			description: icon.name.toLowerCase() + ' (added in v' + icon.created.toFixed(1) + ')',
